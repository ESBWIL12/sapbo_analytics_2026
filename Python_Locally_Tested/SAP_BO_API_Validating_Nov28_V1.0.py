...
class webi_Document: #WEBI Document Object to hold the structured details for the MetaData extraction
    ...
    def save_webi_json(self, folder_path: str): # Save the WEBI document details and data providers to a JSON file
        ...
        file_Name=f"{folder_path}\\WEBI_Document_{self.document_id}.json"
        if os.path.exists(file_Name):
            ...
            while True:
                ...
                count += 1
        try:
            if self.save_webi_excel(folder_path):
    ...
    def save_webi_excel(self, folder_path: str) -> bool:
        ...
        with pd.ExcelWriter(file_name) as writer:
            ...
            DataProvider_to_Excel_df.to_excel(writer, sheet_name='WEBI Metadata', index=False)
...
def main():
    ...
    FolderPath=r"C:\Users\esbwil1\Atradius\Data Platform Team - Squad 1 # Scope & Inventory\04 BO Exploration\CMS Extraction\Python_WEBI_SQL_extraction"
    SourceFileName="Report_list.xlsx"
    ...
    Report_list_filename=f"C:\\Users\\esbwil1\\OneDrive - Atradius\\Documents\\Python Scripts\\Script_OutPut\\{SourceFileName}"
    ...
    webi_list_df = pd.read_excel(Report_list_filename, dtype=str)
    ...
    if log_status is True:
        ...
        try:
            for document in webi_list_df['Report ID']:
                API_CALLS["Document Calls"] = 0
                API_CALLS["Pauses"]=0  # Times
                    ...
                    for dp in data_providers:
                        ...
                        try:
                            ...
                            if DP_Detail.get("dataSourceType") == "fhsql":
                                ...
                                Document_Details(Output, doc, dp, DP_Detail, sql_query, webi_doc)
                            else:
                ...
                result["Number_of_Data_Providers"]=len(data_providers)  
            ...
            API_CALLS["Total Calls"] += 1
            Save_Outputs(FolderPath, Output, DOC_Output)
...
def Save_Outputs(FolderPath: str, Output: list, DOC_Output: list):
                #  , Todyay_Date: str):
